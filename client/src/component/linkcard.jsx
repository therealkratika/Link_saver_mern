...
export function LinkCard({
  ...
  return (
    <div className="relative group w-full text-left bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-md transition-all">
      <div
        onClick={handleOpen}
        ...
        className="absolute inset-0 z-0 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500"
      />
      ...
      <div className="relative z-10 pointer-events-none">
        <div className="flex items-start gap-3 mb-3">
          <img
            ...
            alt=""
            className="w-6 h-6 rounded mt-0.5 object-contain flex-shrink-0"
            onError={(e) => {
          ...
          <button
            ...
            className={`pointer-events-auto flex-shrink-0 p-1 transition-colors ${
              link.isFavorite ? 'text-yellow-500' : 'text-gray-300 hover:text-yellow-400'
            }`}
          >
            <svg
              width="20"
        ...
        {link.tags?.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-3">
            {link.tags.map((tag) => (
              <span
                key={tag}
                className="px-2.5 py-0.5 text-xs font-medium bg-violet-50 text-violet-600 rounded-full border border-violet-100"
              >
                #{tag}
              </span>
            ))}
